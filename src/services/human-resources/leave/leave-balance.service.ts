...
const LEAVE_CODE_TO_BUCKET: Record<string, keyof LeaveBalanceJson> = {
  CL: "casual",
  "LV-CL": "casual",
  SL: "sick",
  "LV-SL": "sick",
...
export function leaveTypeCodeToBucket(code?: string | null): keyof LeaveBalanceJson {
  if (!code) return "casual";
  const normalized = code.toUpperCase().trim();
  if (LEAVE_CODE_TO_BUCKET[normalized]) return LEAVE_CODE_TO_BUCKET[normalized];
  if (normalized.includes("SICK") || normalized.includes("SL")) return "sick";
  if (normalized.includes("EARNED") || normalized.includes("EL")) return "earned";
  if (normalized.includes("CASUAL") || normalized.includes("CL")) return "casual";
  return "casual";
}

async function getEmployeeBalance(employeeId: string): Promise<LeaveBalanceJson> {
  const emp = await hrModel.get<{ leaveBalance?: LeaveBalanceJson }>(hrTables.employees, employeeId);
  return {
...
async function updateEmployeeBalance(
  employeeId: string,
  balance: LeaveBalanceJson,
): Promise<void> {
...
export async function hasConsumptionTransaction(leaveRequestId: string): Promise<boolean> {
  const rows = await hrModel.list<{ id: string }>(hrTables.leaveBalanceTransactions, {
...
export async function applyConsumption(input: {
  ...
  createdBy?: string;
  remarks?: string;
}): Promise<{ transactionId: string; balanceAfter: LeaveBalanceJson }> {
  if (await hasConsumptionTransaction(input.leaveRequestId)) {
    const existing = await hrModel.list<{ id: string; balanceAfter?: LeaveBalanceJson }>(
      hrTables.leaveBalanceTransactions,
      { filters: { leave_request_id: input.leaveRequestId, transaction_type: "CONSUMED" }, limit: 1 },
    );
  ...
  const after: LeaveBalanceJson = { ...before, [bucket]: afterValue };
...
export async function applyDifferential(input: {
  ...
  createdBy?: string;
  remarks?: string;
}): Promise<{ restored: number; consumed: number; balanceAfter: LeaveBalanceJson }> {
