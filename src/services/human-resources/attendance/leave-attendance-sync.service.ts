...
import { classifyDay } from "./calendar-resolver.js";
...
function hasWork(existing: { punchIn?: string | null; attendanceStatus?: string } | null): boolean {
...
export async function syncLeaveToAttendance(
  ...
  effective: EffectiveLeaveResult,
  changedBy?: string,
): Promise<number> {
  const app = await hrModel.get<LeaveApplicationSyncRow>(hrTables.leaveApplications, leaveApplicationId);
...
export async function recalculateDatesAfterLeaveChange(
  propertyId: string,
  employeeId: string,
  dates: string[],
  changedBy?: string,
...
export async function syncLeaveApplicationDiff(input: {
  ...
  changedBy?: string;
}): Promise<{ synced: number; recalculated: number }> {
  ...
  const app = await hrModel.get<LeaveApplicationSyncRow>(
    hrTables.leaveApplications,
    input.leaveApplicationId,
  );
...
export async function recalculateAllDatesInRange(
  propertyId: string,
  employeeId: string,
  fromDate: string,
  toDate: string,
...
export async function syncApprovedLeaveToAttendance(
  ...
  const app = await hrModel.get<LeaveApplicationSyncRow & { durationOption?: string }>(
    hrTables.leaveApplications,
    leaveApplicationId,
  );
  if (!app) throw new Error("Leave application not found");
