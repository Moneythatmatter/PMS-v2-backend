...
export async function resolveShiftForEmployee(
...
): Promise<{ shiftId: string; timing: ShiftTiming; shiftCode: string; shiftName: string }> {
  const assignments = await hrModel.list<ShiftAssignmentRow>(hrTables.shiftAssignments, {
  ...
  if (activeAssignment?.shiftTypeId) {
    const shiftType = await hrModel.get<ShiftTypeRow>(
      hrTables.shiftTypes,
      activeAssignment.shiftTypeId,
    );
  ...
  const employee = await hrModel.get<EmployeeRow>(hrTables.employees, employeeId);
  if (employee?.shiftTypeId) {
    const shiftType = await hrModel.get<ShiftTypeRow>(hrTables.shiftTypes, employee.shiftTypeId);
    if (shiftType) {
      return {
        shiftId: shiftType.id,
  ...
  const defaults = await hrModel.list<ShiftTypeRow>(hrTables.shiftTypes, {
...
export function scheduledHoursForShift(timing: ShiftTiming): number {
  return scheduledHoursFromShift(timing);
}

export function defaultPunchInForShift(attendanceDate: string, timing: ShiftTiming): Date {
