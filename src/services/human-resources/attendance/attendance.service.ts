...
function conflictRemark(existing?: string | null): string {
  const base = existing ?? "";
  if (base.toLowerCase().includes("conflict")) return base;
...
}): { attendanceStatus: string; leaveRequestId: string | null } {
...
function shouldReplaceForCalendarStatus(status?: string): boolean {
  return !status || status === "PENDING" || status === "ABSENT";
}

export async function recalculateAttendanceForDate(
  propertyId: string,
  employeeId: string,
  attendanceDate: string,
  options?: { changedBy?: string; clearLeaveLink?: boolean; now?: Date },
...
export async function writeAttendanceAudit(input: AttendanceAuditInput): Promise<void> {
...
export async function getEmployeeForProperty(
  employeeId: string,
  propertyId: string,
): Promise<{ id: string; propertyId: string; status: string } | null> {
  const emp = await hrModel.get<{ id: string; propertyId: string; status: string }>(
    hrTables.employees,
    employeeId,
  );
  if (!emp || emp.propertyId !== propertyId) return null;
  return emp;
}

export async function findDailyRecord(
  propertyId: string,
  employeeId: string,
  attendanceDate: string,
): Promise<AttendanceRecordRow | null> {
  const rows = await hrModel.list<AttendanceRecordRow>(hrTables.attendanceRecords, {
    filters: {
...
async function baseFields(ctx: BuildContext) {
  ...
  let attendanceStatus: string;
  let holidayWorked = false;
  let leaveRequestId: string | null = classification.approvedLeave?.id ?? null;
...
function recalcFromPunches(
  punchIn: string | null | undefined,
  punchOut: string | null | undefined,
  timing: { breakDurationMinutes: number },
  scheduledHours: number,
): { workedHours: number; extraHours: number } {
  if (!punchIn || !punchOut) {
...
export async function upsertDailyRecord(
  input: Partial<AttendanceRecordRow> & {
    propertyId: string;
    employeeId: string;
    attendanceDate: string;
  },
  options?: { changedBy?: string; auditAction?: string; overrideReason?: string },
  ...
  const { workedHours, extraHours } = recalcFromPunches(
    punchIn,
    punchOut,
    base.timing,
    base.scheduledHours,
  ...
  if (input.attendanceStatus === undefined) {
    if (punchIn && !punchOut) {
      if (base.dayType === "HOLIDAY") attendanceStatus = "PRESENT";
      else if (base.dayType === "WEEKLY_OFF") attendanceStatus = "PRESENT";
  ...
  if (base.dayType === "HOLIDAY" && punchIn) holidayWorked = true;
  ...
  const payload: Record<string, unknown> = {
    ...
    holidayWorked,
    leaveRequestId:
      input.leaveRequestId !== undefined ? input.leaveRequestId : base.leaveRequestId,
  ...
  if (existing) {
    row = await hrModel.update<AttendanceRecordRow>(
    ...
    payload.id = newId();
    row = await hrModel.create<AttendanceRecordRow>(hrTables.attendanceRecords, payload);
    if (options?.auditAction) {
      await writeAttendanceAudit({
        propertyId: input.propertyId,
...
export async function punchIn(input: {
  ...
  if (!emp) throw new Error("Employee not found for this property");
...
export async function correctAttendance(input: {
  ...
  const existing = await hrModel.get<AttendanceRecordRow>(
    hrTables.attendanceRecords,
    input.attendanceId,
  );
...
export async function recalculateAttendance(
  ...
  const row = await recalculateAttendanceForDate(
    existing.propertyId,
    existing.employeeId,
    existing.attendanceDate,
    { changedBy },
  );
  if (!row) throw new Error("Failed to recalculate attendance");
  return row;
}
