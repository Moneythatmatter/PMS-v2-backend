...
export async function ensureSystemBookingTypes(_req: Request, res: Response) {
  try {
    const existing = await smModel.list(smTables.bookingTypes, { orderBy: "sort_order" });
    const existingCodes = new Set(
      existing.map((row) => String((row as Record<string, unknown>).code)),
    );
...
export async function listBookingTypes(_req: Request, res: Response) {
  try {
