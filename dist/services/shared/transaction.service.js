...
function normalizePaymentMethod(mode) {
    ...
    if (/card|credit|debit/i.test(m))
        return "CARD";
    if (/upi|gpay|phonepe|paytm/i.test(m))
        return "UPI";
    if (/bank|neft|rtgs|imps/i.test(m))
        return "BANK_TRANSFER";
    if (/cheque|check/i.test(m))
        return "CHEQUE";
    return "OTHER";
}
/**
...
export const TransactionService = {
    async list(filters = {}) {
        return foModel.list(foModel.tables.transactions, {
            filters: {
    ...
    async get(id) {
        return foModel.get(foModel.tables.transactions, id);
    },
    async create(input) {
        if (!(Number(input.amount) >= 0)) {
            throw new AppError("Transaction amount must be >= 0");
        }
        const row = await foModel.create(foModel.tables.transactions, {
    ...
    async ensureFolioForBooking(bookingId, guestId) {
    ...
    async recordFrontOfficePayment(input) {
        ...
        if (!folioId && input.bookingId) {
            folioId = await this.ensureFolioForBooking(input.bookingId, input.guestId ?? null);
        }
    ...
    async recordReservationAdvance(input) {
        const folioId = await this.ensureFolioForBooking(input.bookingId, input.guestId ?? null);
        return this.recordViaRpc({
            amount: input.amount,
            paymentMethod: normalizePaymentMethod(input.paymentMethod),
            folioId,
