...
import { findDailyRecord, isShiftCutoffPassed, recalculateAttendanceForDate, shouldReplaceForCalendarStatus, upsertDailyRecord, } from "./attendance.service.js";
import { resolveShiftForEmployee } from "./shift-resolver.js";
async function isLeaveStillValid(employeeId, date, leaveRequestId) {
    const live = await findApprovedLeaveForDate(employeeId, date);
    if (!live)
        return false;
    if (leaveRequestId && live.id !== leaveRequestId)
        return false;
    return true;
}
export async function processAbsenceForProperty(propertyId, options) {
    const date = (options?.attendanceDate ?? new Date().toISOString().slice(0, 10)).slice(0, 10);
    const now = options?.now ?? new Date();
    const employees = await hrModel.list(hrTables.employees, { filters: { property_id: propertyId, status: "Active" } });
    let created = 0;
    let skipped = 0;
    for (const emp of employees) {
        if (emp.propertyId !== propertyId)
            continue;
...
//# sourceMappingURL=absence-processor.js.map