...
import { assignmentIncludesDay, assignmentMatchesWeekday, deriveDisplayStatus, isEffectiveOnDate, isExplicitlyDisabled, rangesOverlap, resolveWeeklyOffFromAssignments, } from "./weekly-off.service.js";
...
describe("resolveWeeklyOffFromAssignments", () => {
    it("Sunday assigned resolves WEEKLY_OFF on Sunday", () => {
        const result = resolveWeeklyOffFromAssignments([baseAssignment({})], "2026-09-13");
    ...
    it("Monday does not resolve weekly off for Sunday assignment", () => {
        const result = resolveWeeklyOffFromAssignments([baseAssignment({})], "2026-09-14");
        assert.equal(result.isWeeklyOff, false);
    });
    it("ignores UI Expired label — historical date still resolves", () => {
        const result = resolveWeeklyOffFromAssignments([baseAssignment({ effectiveFrom: "2026-08-01", effectiveTo: "2026-08-31", status: "Expired" })], "2026-08-16");
        assert.equal(result.isWeeklyOff, true);
    });
    it("future assignment ignored before effective_from", () => {
        const result = resolveWeeklyOffFromAssignments([baseAssignment({ effectiveFrom: "2026-10-01", effectiveTo: "2026-10-31" })], "2026-09-13");
        assert.equal(result.isWeeklyOff, false);
    });
    it("expired-by-date assignment ignored for dates after effective_to", () => {
        const result = resolveWeeklyOffFromAssignments([baseAssignment({ effectiveFrom: "2026-08-01", effectiveTo: "2026-08-31" })], "2026-09-13");
        assert.equal(result.isWeeklyOff, false);
    });
    it("explicitly cancelled assignment is ignored even on matching date", () => {
        const result = resolveWeeklyOffFromAssignments([baseAssignment({ status: "Cancelled" })], "2026-09-13");
        assert.equal(result.isWeeklyOff, false);
    });
    it("explicitly inactive assignment is ignored on matching weekday", () => {
        const result = resolveWeeklyOffFromAssignments([baseAssignment({ status: "Inactive", effectiveFrom: "2026-08-01", effectiveTo: "2026-08-31" })], "2026-08-16");
        assert.equal(result.isWeeklyOff, false);
    });
});
describe("rangesOverlap", () => {
    it("non-overlapping ranges do not overlap", () => {
        assert.equal(rangesOverlap("2026-08-01", "2026-08-31", "2026-09-01", "2026-09-30"), false);
    });
    it("partially overlapping ranges overlap", () => {
        assert.equal(rangesOverlap("2026-08-15", "2026-09-15", "2026-09-01", "2026-09-30"), true);
    });
...
//# sourceMappingURL=weekly-off.service.test.js.map