...
function isReportedByFkError(message) {
    return /damage_reports_reported_by_fkey/i.test(message);
}
function appendReporterNote(notes, label) {
...
export function sanitizeDamageReportInput(input) {
    const body = { ...input };
...
export async function persistDamageReportRow(payload, options, reporterLabel) {
    const save = async (body) => {
        if (options.mode === "create") {
            return hkModel.create(hkModel.tables.damageReports, body);
        }
        return hkModel.update(hkModel.tables.damageReports, options.id, body);
    };
    try {
        ...
        if (!isReportedByFkError(message) || !reporterLabel)
            throw error;
        const fallback = {
            ...payload,
            reportedBy: null,
...
function applyEnrichment(row, room, guest, user) {
    return {
        ...row,
...
export async function enrichDamageReport(row) {
    ...
    return applyEnrichment(row, roomId ? roomMap.get(roomId) : undefined, guestId ? guestMap.get(guestId) : undefined, reportedBy ? userMap.get(reportedBy) : undefined);
}
export async function enrichDamageReports(rows) {
    if (!rows.length)
        return [];
    const roomIds = [
    ...
    return rows.map((row) => applyEnrichment(row, row.roomId ? roomMap.get(String(row.roomId)) : undefined, row.guestId ? guestMap.get(String(row.guestId)) : undefined, row.reportedBy ? userMap.get(String(row.reportedBy)) : undefined));
}
export async function resolveDamageReportId(key) {
    const trimmed = key.trim();
    if (!trimmed)
        return null;
    const byId = await hkModel.get(hkModel.tables.damageReports, trimmed);
    if (byId?.id)
        return byId.id;
    const { data, error } = await supabase
...
export async function resolveRoomIdForDamageReport(key) {
...
export async function resolveReporterUserId(input) {
    const raw = String(input ?? "").trim();
    if (!raw || raw === "—")
        return null;
    const { data: byId } = await supabase
...
export async function resolveReporterUserLabel(input) {
    const raw = String(input ?? "").trim();
    if (!raw || raw === "—")
        return "";
    const { data: byId } = await supabase
...
export function parseReportedAt(input, fallback = new Date()) {
    if (input == null || input === "")
...
//# sourceMappingURL=damage-report-enrich.js.map