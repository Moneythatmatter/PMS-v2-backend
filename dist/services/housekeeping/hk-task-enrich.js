...
function applyEnrichment(row, room, booking, users = new Map()) {
    return {
        ...row,
...
export async function enrichHkTask(row) {
    ...
    return applyEnrichment(row, roomMap.get(roomId), bookingId ? bookingMap.get(bookingId) : undefined, userMap);
}
export async function enrichHkTasks(rows) {
    ...
    return rows.map((row) => applyEnrichment(row, roomMap.get(String(row.roomId)), row.bookingId ? bookingMap.get(String(row.bookingId)) : undefined, userMap));
}
