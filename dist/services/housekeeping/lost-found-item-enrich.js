...
function isFoundByFkError(message) {
    return /lost_found_items_found_by_fkey/i.test(message);
}
function appendFoundByNote(notes, label) {
...
export function sanitizeLostFoundItemInput(input) {
    const body = { ...input };
    if (body.item != null && body.itemName == null)
        body.itemName = body.item;
    if (body.name != null && body.itemName == null)
        body.itemName = body.name;
...
export async function persistLostFoundItemRow(payload, options, foundByLabel) {
    const save = async (body) => {
        if (options.mode === "create") {
            return hkModel.create(hkModel.shared.lostFoundItems, body);
        }
        return hkModel.update(hkModel.shared.lostFoundItems, options.id, body);
    };
    try {
        ...
        if (!isFoundByFkError(message) || !foundByLabel)
            throw error;
        const fallback = {
            ...payload,
            foundBy: null,
...
function applyEnrichment(row, room, guest, claimedGuest, user) {
    return {
        ...row,
...
export async function enrichLostFoundItem(row) {
    ...
    return applyEnrichment(row, roomId ? roomMap.get(roomId) : undefined, guestId ? guestMap.get(guestId) : undefined, claimedBy ? guestMap.get(claimedBy) : undefined, foundBy ? userMap.get(foundBy) : undefined);
}
export async function enrichLostFoundItems(rows) {
    if (!rows.length)
        return [];
    const roomIds = [
    ...
    return rows.map((row) => applyEnrichment(row, row.roomId ? roomMap.get(String(row.roomId)) : undefined, row.guestId ? guestMap.get(String(row.guestId)) : undefined, row.claimedBy ? guestMap.get(String(row.claimedBy)) : undefined, row.foundBy ? userMap.get(String(row.foundBy)) : undefined));
}
export async function resolveLostFoundItemId(key) {
    const trimmed = key.trim();
    if (!trimmed)
        return null;
    const byId = await hkModel.get(hkModel.shared.lostFoundItems, trimmed);
    if (byId?.id)
        return byId.id;
    const { data, error } = await supabase
...
export async function resolveRoomIdForLostFound(key) {
    return resolveRoomId(key);
}
export async function resolveGuestIdByName(input) {
    ...
    const exact = (rows ?? []).find((row) => String(row.name ?? "").trim().toLowerCase() === raw.toLowerCase());
    return exact?.id ? String(exact.id) : null;
}
export async function resolveFoundByUserId(input) {
    const raw = String(input ?? "").trim();
    if (!raw || raw === "—")
        return null;
    const { data: byId } = await supabase
...
export async function resolveFoundByUserLabel(input) {
    const raw = String(input ?? "").trim();
    if (!raw || raw === "—")
        return "";
    const { data: byId } = await supabase
...
export function parseFoundAt(input, fallback = new Date()) {
    if (input == null || input === "") {
...
//# sourceMappingURL=lost-found-item-enrich.js.map