...
import { enrichLostFoundItem, enrichLostFoundItems, parseFoundAt, persistLostFoundItemRow, resolveFoundByUserId, resolveFoundByUserLabel, resolveGuestIdByName, resolveLostFoundItemId, resolveRoomIdForLostFound, sanitizeLostFoundItemInput, } from "./lost-found-item-enrich.js";
import { normalizeLostFoundCategory, normalizeLostFoundReturnMethod, normalizeLostFoundStatus, } from "../../types/housekeeping.js";
...
async function getItemOrThrow(id) {
    const row = await hkModel.get(hkModel.shared.lostFoundItems, id);
    if (!row)
...
export const LostFoundItemService = {
    async list(filters = {}) {
        ...
        let rows = await hkModel.list(hkModel.shared.lostFoundItems, {
            filters: queryFilters,
            orderBy: "found_at",
            ascending: false,
        });
        if (filters.roomId) {
            const roomId = (await resolveRoomIdForLostFound(filters.roomId)) ?? filters.roomId;
    ...
    async create(input) {
        ...
        if (!itemName)
            throw new AppError("itemName is required", 400);
        ...
        const foundByLabel = (await resolveFoundByUserLabel(foundByRaw)) || foundByRaw;
        ...
        const saved = await persistLostFoundItemRow({
            ...
            guest: undefined,
        }, { mode: "create" }, foundByLabel || undefined);
    ...
    async update(id, input) {
        ...
        if (CLOSED_STATUSES.includes(existing.status)) {
            throw new ConflictError(`Cannot edit item in status ${existing.status}`);
        }
        const body = sanitizeLostFoundItemInput(input);
        const patch = {};
        ...
        if (body.storedLocation != null)
            patch.storedLocation = body.storedLocation;
        if (body.notes != null)
        ...
        if (body.foundBy != null) {
            ...
            patch.foundBy = foundByRaw
                ? await resolveFoundByUserId(foundByRaw)
                : null;
            const saved = await persistLostFoundItemRow(patch, { mode: "update", id: resolved }, foundByLabel || undefined);
    ...
    async returnItem(id, input = {}) {
        ...
        const returnedTo = String(input.returnedTo ?? input.claimBy ?? input.guest ?? "").trim();
        const nowIso = new Date().toISOString();
        const returnMethod = normalizeLostFoundReturnMethod(input.returnMethod ?? "IN_PERSON");
        const saved = await persistLostFoundItemRow({
            status: "RETURNED",
    ...
    async claimItem(id, input = {}) {
        ...
        const claimant = String(input.claimedBy ?? input.claimBy ?? input.guest ?? input.returnedTo ?? "").trim();
        ...
        const saved = await persistLostFoundItemRow({
            ...
            claimedAt: nowIso,
            returnMethod: normalizeLostFoundReturnMethod(input.returnMethod) ?? "IN_PERSON",
            notes: input.notes ?? existing.notes,
        }, { mode: "update", id: resolved });
        const row = await enrichLostFoundItem({
            ...saved,
    ...
    async courierDispatch(id, input = {}) {
        const resolved = await resolveLostFoundItemId(id);
        if (!resolved)
            throw new NotFoundError("Lost & found item not found");
        const saved = await persistLostFoundItemRow({
...
//# sourceMappingURL=lost-found-item.service.js.map