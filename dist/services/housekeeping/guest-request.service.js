...
import { enrichGuestRequest, enrichGuestRequests, persistGuestRequestRow, resolveGuestRequestAssignee, resolveGuestRequestAssigneeLabel, resolveGuestRequestId, resolveRoomIdForGuestRequest, sanitizeGuestRequestInput, } from "./guest-request-enrich.js";
import { normalizeGuestRequestPriority, normalizeGuestRequestStatus, normalizeGuestRequestType, } from "../../types/housekeeping.js";
...
async function getRequestOrThrow(id) {
    const row = await hkModel.get(hkModel.tables.guestRequests, id);
    if (!row)
        throw new NotFoundError("Guest request not found");
    return row;
}
...
export const GuestRequestService = {
    async list(filters = {}) {
        ...
        let rows = await hkModel.list(hkModel.tables.guestRequests, {
            filters: queryFilters,
            orderBy: "requested_at",
            ascending: false,
        });
        if (filters.roomId) {
            const roomId = (await resolveRoomIdForGuestRequest(filters.roomId)) ?? filters.roomId;
    ...
    async create(input) {
        ...
        const assigneeLabel = (await resolveGuestRequestAssigneeLabel(assignedToRaw)) || assignedToRaw;
    ...
    async update(id, input) {
        ...
        if (body.description != null) {
            ...
            patch.requestType = normalizeGuestRequestType(body.requestType ?? description);
        }
        ...
        const saved = await hkModel.update(hkModel.tables.guestRequests, resolved, patch);
    ...
    async assign(id, assignedTo) {
        ...
        }, { mode: "update", id: resolved }, assigneeLabel);
        const row = await enrichGuestRequest({
            ...saved,
    ...
    async start(id) {
        ...
        return enrichGuestRequest(await hkModel.update(hkModel.tables.guestRequests, resolved, {
            status: "IN_PROGRESS",
        }));
    },
    async complete(id, notes) {
        ...
        const row = await enrichGuestRequest(await hkModel.update(hkModel.tables.guestRequests, resolved, {
            status: "COMPLETED",
            completedAt: now,
...
//# sourceMappingURL=guest-request.service.js.map