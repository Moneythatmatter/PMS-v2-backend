...
async function loadBookingSources() {
    ...
    sourceCache = (data ?? []).map((row) => toCamel(row));
    return sourceCache;
}
/** Resolve booking source UUID, code, or display name → booking_sources.id */
export async function resolveSourceId(ref) {
    ...
    const aliasCode = normalized === "direct" || normalized === "website"
        ? "web"
        : normalized === "walk-in" || normalized === "walkin"
            ...
            : normalized === "booking.com"
                ...
                : normalized === "makemytrip"
                    ...
                    : null;
    if (aliasCode) {
        const match = sources.find((s) => String(s.code ?? "").toLowerCase() === aliasCode);
        if (match)
            return match.id;
    }
    return null;
}
export async function fetchBookingSourcesByIds(ids) {
    const map = new Map();
...
export async function defaultWalkInSourceId() {
    const sources = await loadBookingSources();
