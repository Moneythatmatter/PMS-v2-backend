...
async function recomputeOrderAmount(orderId) {
...
function isBillableOrderItem(status) {
...
async function assertBillAdjustable(orderId) {
    ...
    if (paymentStatus === "PAID" || paymentStatus === "PARTIALLY_PAID") {
        throw new AppError("Cannot cancel items on a paid or partially paid bill. Process a refund or adjustment first.", 400);
    }
}
async function voidOrderItemQuantity(orderItemId, voidQty) {
    const orderItem = await fbModel.get(fbModel.tables.orderItems, orderItemId);
    if (!orderItem || !isBillableOrderItem(orderItem.status))
        return;
...
async function syncOrderLifecycleFromItems(orderId) {
    const items = await getOrderItems(orderId);
...
async function maybeCancelKotTicket(kotId) {
    const kotItems = await fbModel.list(fbModel.tables.kotItems, {
        filters: { kot_id: kotId },
    });
    if (!kotItems.length)
        return;
    const allCancelled = kotItems.every((item) => String(item.status).toUpperCase() === "CANCELLED");
    if (!allCancelled)
        return;
...
async function ensureBillForOrder(orderId, subtotal) {
    const existing = await fbModel.list(fbModel.tables.bills, {
        filters: { order_id: orderId },
        limit: 1,
    });
    if (existing[0]?.id) {
        ...
        if (paymentStatus === "PAID" || paymentStatus === "PARTIALLY_PAID")
            return bill;
        return fbModel.update(fbModel.tables.bills, String(bill.id), {
            subtotal,
    ...
    return fbModel.create(fbModel.tables.bills, {
        id: fbModel.newId("BL"),
        orderId,
        status: "OPEN",
...
async function findLiveTable(outletId, tableNo) {
    const normalized = tableNo.trim().toLowerCase();
    if (!outletId || !normalized)
        return null;
    const rows = await fbModel.list(fbModel.tables.liveTables, {
        filters: { outlet_id: outletId },
    });
    return (rows.find((t) => String(t.tableNo ?? "").toLowerCase() === normalized) ??
        null);
}
async function getOpenSessionForTable(liveTableId) {
    const rows = await fbModel.list(fbModel.tables.tableSessions, {
        filters: { live_table_id: liveTableId, status: "OPEN" },
        limit: 1,
    });
    return rows[0] ?? null;
}
async function getOpenOrderForSession(sessionId) {
...
async function createOrderRecord(input) {
    return fbModel.create(fbModel.tables.orders, {
        id: fbModel.newId("OR"),
        outletId: input.outletId,
        type: input.type,
        ref: input.ref,
...
export const PosService = {
    async sendKot(input) {
        ...
        let liveTable = null;
        if (input.orderId) {
            order = await fbModel.get(fbModel.tables.orders, input.orderId);
            if (!order)
            ...
            liveTable = input.liveTableId
                ? await fbModel.get(fbModel.tables.liveTables, input.liveTableId)
        ...
        if (!order) {
            ...
            const patch = {};
            if (input.guest)
                patch.guest = input.guest;
            if (input.server)
                patch.server = input.server;
            if (input.pax)
                patch.pax = input.pax;
        ...
        const kot = await fbModel.create(fbModel.tables.kotTickets, {
            id: fbModel.newId("KT"),
            orderId,
            status: "PREPARING",
        ...
        const fullOrder = await fbModel.get(fbModel.tables.orders, orderId);
    ...
    async getOrderWithDetails(orderId) {
        ...
        const [items, kots, bills] = await Promise.all([
            ...
            fbModel.list(fbModel.tables.kotTickets, {
                filters: { order_id: orderId },
                orderBy: "created_at",
            }),
            fbModel.list(fbModel.tables.bills, {
                filters: { order_id: orderId },
                orderBy: "created_at",
            }),
        ]);
        const kotsWithItems = await Promise.all(kots.map(async (k) => ({
            ...k,
    ...
    async getOpenOrderForTable(liveTableId) {
    ...
    async printBill(billId) {
        const bill = await fbModel.get(fbModel.tables.bills, billId);
        if (!bill)
    ...
    async printBillForOrder(orderId) {
        const bills = await fbModel.list(fbModel.tables.bills, {
            filters: { order_id: orderId },
            limit: 1,
        });
    ...
    async syncBillPaymentStatus(billId) {
        const bill = await fbModel.get(fbModel.tables.bills, billId);
        if (!bill)
            throw new AppError("Bill not found", 404);
    ...
    async payBillFull(input) {
        ...
        if (remaining <= 0) {
            const bill = (await this.syncBillPaymentStatus(input.billId));
            const orderId = String(bill.orderId ?? "");
            return {
                bill,
    ...
    async payOrderLegacy(orderId, paymentMethod) {
        const bills = await fbModel.list(fbModel.tables.bills, {
            filters: { order_id: orderId },
            limit: 1,
        });
    ...
    async advanceKot(kotId) {
        const kot = await fbModel.get(fbModel.tables.kotTickets, kotId);
        if (!kot)
            throw new AppError("KOT not found", 404);
        const flow = ["PENDING", "PREPARING", "READY", "SERVED"];
        const idx = flow.indexOf(String(kot.status));
        ...
        await Promise.all(kotItems
            .filter((i) => String(i.status).toUpperCase() !== "CANCELLED")
            .map((i) => fbModel.update(fbModel.tables.kotItems, String(i.id), { status: next })));
        const orderId = String(kot.orderId ?? "");
        if (orderId) {
            const kots = await fbModel.list(fbModel.tables.kotTickets, {
                filters: { order_id: orderId },
            });
    ...
    async syncOrderStatusFromKots(orderId) {
        const kots = await fbModel.list(fbModel.tables.kotTickets, {
            filters: { order_id: orderId },
        });
        const orderStatus = deriveOrderStatusFromKots(kots);
    ...
    async buildKotRow(kot, orderById) {
        const order = orderById.get(String(kot.orderId ?? ""));
        if (!order)
            return null;
        const orderId = String(kot.orderId ?? "");
        const [kotItems, orderItems] = await Promise.all([
            fbModel.list(fbModel.tables.kotItems, {
                filters: { kot_id: String(kot.id) },
            }),
            getOrderItems(orderId),
        ]);
        const orderItemById = new Map(orderItems.map((item) => [String(item.id), item]));
    ...
    async listKots(outletId) {
        ...
        const scoped = kots.filter((kot) => {
            ...
            if (!order)
                return false;
            if (outletId && String(order.outletId ?? "") !== outletId)
                return false;
        ...
        const refreshedKots = scoped.some((k) => String(k.status ?? "").toUpperCase() === "PENDING")
            ? await fbModel.list(fbModel.tables.kotTickets, {
                orderBy: "created_at",
                ascending: false,
            })
            : kots;
        const rows = await Promise.all(refreshedKots
            .filter((kot) => {
            const order = orderById.get(String(kot.orderId ?? ""));
            if (!order)
                return false;
    ...
    async acceptKot(kotId, prepMinutes) {
        const kot = await fbModel.get(fbModel.tables.kotTickets, kotId);
        if (!kot)
            throw new AppError("KOT not found", 404);
        if (String(kot.status).toUpperCase() !== "PENDING") {
        ...
        await Promise.all(kotItems.map((item) => fbModel.update(fbModel.tables.kotItems, String(item.id), {
            status: "PREPARING",
        })));
        const orderId = String(kot.orderId ?? "");
        if (orderId) {
            const patch = { rejectReason: null };
        ...
        const orderById = new Map(orders.map((order) => [String(order.id), order]));
        const updatedKot = await fbModel.get(fbModel.tables.kotTickets, kotId);
        if (!updatedKot)
            throw new AppError("KOT not found", 404);
    ...
    async rejectKot(kotId, reason) {
        ...
        for (const item of kotItems) {
            ...
            await voidOrderItemQuantity(String(item.orderItemId), Number(item.quantity ?? 1));
        }
        await fbModel.update(fbModel.tables.kotTickets, kotId, {
            status: "CANCELLED",
            updatedAt: nowIso(),
    ...
    async cancelKotItem(kotItemId, reason) {
        const kotItem = await fbModel.get(fbModel.tables.kotItems, kotItemId);
        if (!kotItem)
            throw new AppError("KOT item not found", 404);
        ...
        await voidOrderItemQuantity(String(kotItem.orderItemId), Number(kotItem.quantity ?? 1));
    ...
    async syncKitchenFromOrder(orderId, orderStatus) {
        ...
        for (const kot of kots) {
            if (String(kot.status) === "CANCELLED")
                continue;
...
function deriveOrderStatusFromKots(kots) {
    if (!kots.length)
        return null;
    const statuses = kots
