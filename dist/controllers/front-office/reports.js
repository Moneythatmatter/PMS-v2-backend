...
export async function getReport(req, res) {
    try {
        ...
        const [reservationsRaw, rooms, payments, shifts] = await Promise.all([
            ...
            foModel.list(foModel.tables.payments),
        ...
        const roomRevenue = reservations.reduce((s, r) => s + Number(r.totalAmount ?? 0), 0);
        const paymentTotal = payments.reduce((s, p) => s + Number(p.amount ?? 0), 0);
        const occupied = reservations.filter((r) => r.status === "Checked In" || r.status === "In-House").length;
                ...
                case "occupancy":
                    return rooms.map((r) => {
                        ...
                        return {
                            ...
                            type: r.roomType,
                            floor: r.floor,
                            status: deriveFoRoomStatus(hkStatus, booking, r.isActive !== false),
                            guest: booking
