...
async function findProductById(id) {
    if (!id?.trim())
        return null;
    const rows = await listRows(T.products, { filters: { id }, limit: 1 });
    return rows[0] ?? null;
}
async function findProductByCode(code) {
    if (!code?.trim())
        return null;
    const rows = await listRows(T.products, { filters: { product_code: code }, limit: 1 });
    return rows[0] ?? null;
}
...
async function findWarehouseByName(name) {
    const rows = await listRows(T.warehouses, { limit: 50 });
...
export async function postGrnStock(params) {
    ...
    for (const line of params.items) {
        ...
        for (const batch of allocations) {
            ...
            if (qtyIn <= 0)
                continue;
            const balanceQty = await upsertStockBalance(product.id, wh.id, qtyIn, Number(line.unitRate ?? 0));
...
function flattenGrnItemsForQc(items) {
...
export async function createGrn(req, res) {
    try {
        ...
        if (!grnPayload.id)
            grnPayload.id = newId("GRN");
        const grn = await insertRow(T.grns, grnPayload);
...
export async function updateQualityInspection(req, res) {
    try {
        const id = String(req.params.id);
        const body = { ...req.body };
        delete body.id;
        const prev = await getRowById(T.qualityInspections, id);
        if (!prev)
            return fail(res, "Not found", 404);
        const row = await updateRow(T.qualityInspections, id, body);
        ...
        if (wasCompleted && prev.grnNumber) {
            const grns = await listRows(T.grns, {
            ...
            if (grn) {
                ...
                const qiItems = body.items ?? [];
                const mergedItems = items.map((line) => {
                    const qi = qiItems.find((q) => q.materialId && q.materialId === line.materialId) ??
                    ...
                    return {
                        ...
                        acceptedQty,
                        rejectedQty,
                        qcStatus: rejectedQty > 0 && acceptedQty > 0
                            ? "Partial"
                            : acceptedQty > 0
                                ? "Passed"
                                : "Failed",
                        batchAllocations: (line.batchAllocations ?? []).map((b) => {
                            ...
                            return {
                                ...
                                qcStatus: batchAccepted > 0 ? "Passed" : "Failed",
                            };
                        }),
                    };
                });
