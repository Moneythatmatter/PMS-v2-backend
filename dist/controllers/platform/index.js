...
export async function listProperties(req, res) {
    try {
        ...
        const rows = await PropertyService.listForUser(auth.userId, auth.isSuperAdmin, auth.role);
        return ok(res, rows);
    }
    catch (e) {
...
export async function updateUser(req, res) {
    try {
        ...
        const row = await UserAdminService.updateUser(String(req.params.id), {
            ...
            status: body.status != null ? String(body.status) : undefined,
            isSuperAdmin: body.isSuperAdmin != null ? Boolean(body.isSuperAdmin) : undefined,
            propertyIds: Array.isArray(body.propertyIds)
                ? body.propertyIds.map(String)
                : undefined,
...
export async function myPermissions(req, res) {
    try {
        ...
        const allowed = await PropertyService.userCanAccessProperty(auth.userId, propertyId, auth.isSuperAdmin, auth.role);
        if (!allowed) {
