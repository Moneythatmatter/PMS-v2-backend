...
export async function getDashboard(_req, res) {
    try {
        const [requestsRaw, workOrdersRaw, pmRaw] = await Promise.all([
            listRows(mntTables.requests, { orderBy: "created_at", ascending: false }),
        ...
        const requests = requestsRaw.map((r) => unpackMntDocument(r));
        const workOrders = workOrdersRaw.map((r) => unpackMntDocument(r));
        const pmSchedules = pmRaw.map((r) => unpackMntDocument(r));
        ...
        const critical = workOrders.filter((w) => activeWoStatuses.has(String(w.status ?? "")) &&
        ...
        const roomsUnder = workOrders.filter((w) => activeWoStatuses.has(String(w.status ?? "")) &&
        ...
        return ok(res, {
            stats: {
                openRequests: {
                    ...
                    inReviewCount: openRequests.filter((r) => ["Verification", "In Review", "Verified"].includes(String(r.status))).length,
                },
                activeWorkOrders: {
