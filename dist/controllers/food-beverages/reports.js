...
function parseMoney(value) {
    ...
    if (upper.endsWith("L"))
        return (Number.parseFloat(upper) || 0) * 100_000;
    if (upper.endsWith("K"))
        return (Number.parseFloat(upper) || 0) * 1_000;
    return Number.parseFloat(upper) || 0;
}
function outletName(outlets, id) {
    const match = outlets.find((o) => o.id === id);
    return match ? String(match.name ?? id ?? "—") : String(id ?? "—");
}
function settledOrders(orders) {
...
function inDateRange(isoDay, from, to) {
...
export async function getReport(req, res) {
    try {
        ...
        const [ordersAll, outletsAll, ingredients, shiftsAll, tablesAll, kdsAll, discounts] = await Promise.all([
            ...
            fbModel.list(fbModel.tables.kdsTickets),
            fbModel.list(fbModel.tables.discounts),
        ]);
        const diningOutletsAll = outletsAll.filter((o) => ["restaurant", "cafe", "bar", "banquet"].includes(String(o.type ?? "")));
        ...
        const orders = ordersAll.filter((o) => {
            if (outletIdFilter && String(o.outletId) !== outletIdFilter)
                return false;
        ...
        const outletSalesFallback = (o) => parseMoney(o.sales);
        const orderSalesByOutlet = (outletId) => settled
        ...
        const salesFromOutlets = diningOutlets.reduce((s, o) => s + outletSalesFallback(o), 0);
        const salesTotal = salesFromOrders > 0 ? salesFromOrders : salesFromOutlets;
        const coversTotal = settled.reduce((s, o) => s + (Number(o.covers ?? 0) || 1), 0) ||
            ...
            switch (type) {
                case "daily-sales": {
                    ...
                    return days.map(([date, v], i) => {
                        ...
                        const growth = prev && prev.sales > 0
                ...
                case "outlet-sales":
                    return diningOutlets.map((o) => {
                        ...
                        const amount = fromOrders > 0 ? fromOrders : outletSalesFallback(o);
                        const outletOrders = settled.filter((ord) => ord.outletId === o.id);
                        const bills = outletOrders.length ||
                            (amount > 0 ? Math.max(1, Number(o.tables ?? 0) || 1) : 0);
                        const covers = outletOrders.reduce((s, ord) => s + (Number(ord.covers ?? 0) || 1), 0) || Number(o.covers ?? 0);
                        return {
                            id: String(o.id),
                            outlet: o.name,
                            bills,
                ...
                case "cashier":
                    return shifts.map((s) => {
                        const sales = Number(s.cashSales ?? 0) +
                            Number(s.cardSales ?? 0) +
                            Number(s.upiSales ?? 0);
                        const declared = s.declaredCash;
                        const expected = Number(s.openingFloat ?? 0) +
                            Number(s.cashSales ?? 0) -
                            Number(s.refunds ?? 0);
                        const variance = declared === null || declared === undefined
                            ? "—"
                ...
                case "table-turnover":
                    return tables.map((t) => {
                        ...
                        const turns = duration > 0 ? Math.max(1, Math.round(480 / duration)) : 1;
                ...
                case "food-cost": {
                    const stockValue = ingredients.reduce((s, i) => s + Number(i.onHand ?? 0) * 100, 0);
                    return diningOutlets.map((o) => {
                        const fromOrders = orderSalesByOutlet(o.id);
                        const amount = fromOrders > 0 ? fromOrders : outletSalesFallback(o);
                        const share = salesTotal > 0
                            ? amount / salesTotal
                ...
                case "inventory": {
                    const low = ingredients.filter((i) => Number(i.onHand ?? 0) <= Number(i.reorder ?? 0)).length;
                    const value = ingredients.reduce((s, i) => s + Number(i.onHand ?? 0) * 100, 0);
                    return [
                        {
                            id: "STORE-MAIN",
                            store: "Main Store",
                ...
                case "kitchen-performance": {
                    ...
                    return source.map((k) => {
                        ...
                        const completed = tickets.filter((t) => ["Ready", "Served", "Completed", "Done"].includes(String(t.status ?? "")));
                        const avg = tickets.length > 0
                            ? tickets.reduce((s, t) => s + Number(t.elapsedMin ?? 0), 0) /
                                tickets.length
                            : 0;
                        const over = tickets.filter((t) => Number(t.elapsedMin ?? 0) > Number(t.slaMin ?? 15)).length;
                        const sla = tickets.length > 0
                ...
                case "cancelled-bills":
                    return orders
                        .filter((o) => ["Cancelled", "Void", "Voided"].includes(String(o.status ?? "")))
                        .map((o) => ({
                        id: String(o.id),
        ...
        const billSum = rows.reduce((s, r) => s + (Number(r.bills ?? 0) || 0), 0);
        const coversSum = rows.reduce((s, r) => s + (Number(r.covers ?? 0) || 0), 0);
        const salesSum = rows.reduce((s, r) => s +
            (Number(r.salesValue ?? 0) || parseMoney(r.sales) || parseMoney(r.amount) || parseMoney(r.revenue)), 0);
