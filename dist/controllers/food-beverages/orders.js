...
async function occupyLiveTableForOrder(order) {
    if (String(order.type ?? "") !== "Dine In")
        return;
    const outletId = String(order.outletId ?? "");
    const ref = String(order.ref ?? "").trim();
    if (!isDineInTableRef(ref) || !outletId)
        return;
    const table = await findLiveTable(outletId, ref);
    if (!table?.id)
        return;
    const pax = Number(order.pax ?? 0);
    const covers = Number.isFinite(pax) && pax > 0
        ? pax
...
async function releaseLiveTableForOrder(order) {
    if (String(order.type ?? "") !== "Dine In")
        return;
    const outletId = String(order.outletId ?? "");
    const ref = String(order.ref ?? "").trim();
    ...
    try {
        ...
        try {
            ...
            if (status !== "Served" && status !== "Ready") {
                return fail(res, "Only Served or Ready orders can be paid. Mark Served on Orders first.", 400);
            }
            const row = await fbModel.update(fbModel.tables.orders, id, {
                status: "Settled",
            ...
            await releaseLiveTableForOrder(row);
            return ok(res, row);
        }
    }
    catch (e) {
