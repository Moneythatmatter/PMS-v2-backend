...
export async function getReport(req, res) {
    try {
        ...
        const [rooms, laundry, inventory, staff, damage, history, publicAreas] = await Promise.all([
            hkModel.list(hkModel.tables.rooms),
            hkModel.list(hkModel.tables.laundryJobs),
            hkModel.list(hkModel.tables.inventory),
            hkModel.list(hkModel.tables.staff),
            hkModel.list(hkModel.tables.damageReports),
            ...
            switch (type) {
                ...
                case "inventory":
                    return inventory.map((i) => ({
                        ...
                        belowPar: Number(i.available ?? 0) < Number(i.parStock ?? 0)
                            ? "Yes"
                            : "No",
                        unit: i.unit,
                    }));
